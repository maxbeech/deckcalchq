...
export async function POST() {
  ...
  const base = process.env.NEXT_PUBLIC_SITE_URL ?? "https://deckcalchq.vercel.app";

  if (!secret || !price) {
    return NextResponse.json(
      { error: "Pro deck plans are launching shortly. Email hello@deckcalchq.com for early access." },
      { status: 503 },
    );
  }
